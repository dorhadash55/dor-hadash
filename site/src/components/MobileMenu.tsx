...
const navIcons: Record<string, ReactNode> = {
  ...
  "/#piliers": (
    <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" aria-hidden="true">
      <path strokeLinecap="round" strokeLinejoin="round" d="M4 20h16M6 20V8l6-4 6 4v12M10 20v-6h4v6" />
    </svg>
  ),
  "/nos-villes": (
    <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" aria-hidden="true">
    ...
    return (
      ...
      >
        ...
        >
          ...
          <span
            className={`flex h-7 w-7 items-center justify-center rounded-full bg-white/10 transition-transform duration-300 ${expanded ? "rotate-180" : ""}`}
          >
    ...
    >
      {({ isActive }) => (
        <>
          ...
          <ChevronRight
            className={isActive && !isHash ? "text-brand-blue/40" : "text-white/35 group-hover:text-white/60"}
          />
        </>
      )}
    </NavLink>
        ...
        <div className="mobile-menu-bg relative flex min-h-0 flex-1 flex-col overflow-hidden">
          ...
          <div className="mobile-menu-footer relative shrink-0 px-5 py-5 pb-[calc(1.25rem+env(safe-area-inset-bottom,0px))]">
            <NavLink
              ...
              onClick={handleClose}
              className="mobile-menu-cta-primary flex items-center justify-center gap-2 rounded-2xl py-4 font-heading text-base font-semibold shadow-xl transition-transform active:scale-[0.98]"
            >
